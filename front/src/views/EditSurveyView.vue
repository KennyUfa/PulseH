...
const form = reactive({ title: '', description: '', is_anonymous: false, status: 'draft', start_date: '', end_date: '', questions: [] })
...
onMounted(async () => {
  try {
    ...
    form.start_date = data.start_date ? data.start_date.slice(0, 16) : ''
    form.end_date = data.end_date ? data.end_date.slice(0, 16) : ''
